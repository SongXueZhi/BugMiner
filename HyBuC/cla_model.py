...
class PatchNetExtended(nn.Module):
    def __init__(self, args):
        ...
        self.convs_msg = nn.ModuleList(
            [nn.Conv2d(Ci, Co, (K, Dim)) for K in Ks])
        ...
        self.convs_code_line = nn.ModuleList(
            [nn.Conv2d(Ci, Co, (K, Dim)) for K in Ks])
        self.convs_code_hunk = nn.ModuleList(
            [nn.Conv3d(Ci, Co, (K, code_line, Co * len(Ks))) for K in Ks])

        # other information
        self.dropout = nn.Dropout(args.dropout_keep_prob)
        self.fc1 = nn.Linear(len(Ks) * Co + 2 * 2 * len(Ks)
                             * Co + Embedding, args.hidden_units)  # hidden units
    ...
    def forward_msg(self, x, convs):
        ...
        x = [F.relu(conv(x)).squeeze(3)
             for conv in convs]  # [(N, Co, W), ...]*len(Ks)
    ...
    def forward_code(self, x):
        ...
        for b in range(0, n_batch):
            ...
            x_i = [F.relu(conv(x_i)).squeeze(3).squeeze(3)
                   for conv in self.convs_code_hunk]
