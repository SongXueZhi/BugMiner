...
def mini_batches(X_added_code, X_removed_code, Y, mini_batch_size=64, seed=0, shuffled=True):
    ...
    for k in range(0, num_complete_minibatches):
        ...
        mini_batch_X_removed = shuffled_X_removed[k * mini_batch_size: k *
                                                  mini_batch_size + mini_batch_size, :, :, :, :]
        if len(Y.shape) == 1:
...
def mini_batches_PNExtended(X_ftr, X_msg, X_added_code, X_removed_code, Y, shuffled=False, mini_batch_size=64, seed=0):
    ...
    for k in range(0, num_complete_minibatches):
        ...
        mini_batch = (mini_batch_X_ftr, mini_batch_X_msg,
                      mini_batch_X_added, mini_batch_X_removed, mini_batch_Y)
        mini_batches.append(mini_batch)
    ...
    if m % mini_batch_size != 0:
        ...
        mini_batch_X_msg = shuffled_X_msg[num_complete_minibatches *
                                          mini_batch_size: m, :]
        mini_batch_X_added = shuffled_X_added[num_complete_minibatches *
                                              mini_batch_size: m, :, :, :]
        mini_batch_X_removed = shuffled_X_removed[num_complete_minibatches *
                                                  mini_batch_size: m, :, :, :]
        if len(Y.shape) == 1:
