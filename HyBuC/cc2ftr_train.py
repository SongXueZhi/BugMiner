...
from cc2ftr_model import HierachicalRNN
from cc2ftr_utils import mini_batches, save


def train_model(data, params):
    pad_added_code, pad_removed_code, pad_msg_labels, dict_msg, dict_code, bert_data = data
    ...
    for epoch in range(1, params.num_epochs + 1):
        ...
        for i, (batch) in enumerate(tqdm(batches)):
            ...
            predict = model.forward(
                pad_added_code, pad_removed_code, state_hunk, state_sent, state_word)
            loss = criterion(predict, labels)
            loss.backward()
            total_loss += loss.item()
            optimizer.step()

        print('Training: Epoch %i / %i -- Total loss: %f' %
